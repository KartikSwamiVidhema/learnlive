...
const handleAdd = async () => {
  ...
  try {
    ...
    if (coverImages instanceof File) {
      coverImageUrl = await uploadToCloudinary(coverImages);
      if (!coverImageUrl) {
        setLoading(false);
        return;
      }
    ...
    if (fileupload instanceof File) {
      fileUrl = await uploadToCloudinary(fileupload);
      if (!fileUrl) {
        setLoading(false);
        return;
      }
      ...
      <div className="md:w-[99%] mx-auto">
        <Card className="rounded-xl shadow-md">
          ...
          <Dialog className="rounded-lg max-h-[500px] overflow-y-scroll" open={isOpen} onOpenChange={setIsOpen}>
              ...
              className="mb-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-4 py-2"
              disabled={loading}
            >
...
const ViewOrderDetails = ({ order, onClose }) => {
  ...
  <p className="font-semibold text-gray-800">{order.customer?.email || "N/A"}</p>
      ...
      <button
        onClick={onClose}
        className="mt-8 w-full md:w-auto px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition duration-300"
      >
        Back to Orders
      </button>
    </div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (storedUserData) {
        ...
        const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
        try {
          const responseProduct = await axios.get(`${apiBaseUrl}/order?filter={"customer":"${userdata._id}"}`);
          console.log('responseProduct', responseProduct.data.data);
          setData(responseProduct.data.data); // Set the fetched data to state
          setTotalPages(Math.ceil(responseProduct.data.data.length / itemsPerPage)); // Calculate total pages
        } catch (error) {
          console.error("Error fetching product data", error);
        }
      }
    ...
    <div className="container mx-auto p-3">
      <div className="md:w-[99%] mx-auto">
        <Card className="rounded-xl shadow-md">
          <CardContent className="p-6">
            {selectedOrder ? (
              // Render the ViewOrderDetails component if an order is selected
              <ViewOrderDetails
                order={selectedOrder}
                onClose={handleCloseOrderDetails}
              />
            ) : (
              // Render the table if no order is selected
              <>
                <Table className="mt-4 rounded-lg border border-gray-200 shadow-md">
                  <TableHeader>
                    <TableRow className="bg-gray-100 rounded-t-lg">
                          ...
                          <TableRow key={index} className="hover:bg-gray-50">
                            ...
                            <TableCell className="p-3 flex space-x-2">
                              <Button
                                ...
                                onClick={() => handleViewOrder(item)} // Handle "View" button click
                              >
                                View
                              </Button>
                            </TableCell>
